...
def emd_approx(sample, ref):
    ...
    # emd = earth_mover_distance_nograd(sample, ref, transpose=False)
    return emd


# Borrow from https://github.com/ThibaultGROUEIX/AtlasNet
def distChamfer(a, b):
    # TODO rick: can be replaced with pytorch3d implementation, which is faster and more efficient
    x, y = a, b
    bs, num_points, points_dim = x.size()  # (b, n, d)
    xx = torch.bmm(x, x.transpose(2, 1))  # (b, n, n)
    yy = torch.bmm(y, y.transpose(2, 1))  # (b, m, m)
    zz = torch.bmm(x, y.transpose(2, 1))  # (b, n, m)
    diag_ind = torch.arange(0, num_points).to(a).long()  # (n,)
    rx = xx[:, diag_ind, diag_ind].unsqueeze(1).expand_as(xx)  # (b, n, n)
    ry = yy[:, diag_ind, diag_ind].unsqueeze(1).expand_as(yy)  # (b, m, m)
...
def normalize_point_clouds(pcs, mode=None):
    if mode is None:
        return pcs
...
if __name__ == "__main__":
    ...
    print(compute_all_metrics(a, a, batch_size=8))
